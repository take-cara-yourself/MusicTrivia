...
import java.io.*;
 ...
 public class KANYEWEST implements questions
 {
  String artist = "KANYEWEST";
  boolean asked = false;
  ...
  // Post: If the name is the same returns 1, if not returns 0
   public int artistName(String guess)
   {
    ...
    if (guess.equals(artist) || guess.equals("KANYE"))
    {
     return(1);
    }
    else
   ...
   public void playSound()
   {
    ...
    try
    {
     BGM = new AudioStream(new FileInputStream("KANYEWEST.wav"));// Imports audio file
     MD =  BGM.getData();
	 ...
	 {
	  System.out.println("Error 401 File Not Found");// Exception if audio file is not found
	 }
   }   
