...
import java.io.*;
 ...
 public class BRITNEYSPEARS implements questions
 {
  String artist = "BRITNEYSPEARS";
  boolean asked = false;
  ...
  // Post: If the name is the same returns 1, if not returns 0
   public int artistName(String guess)
   {
    ...
    if (guess.equals(artist) || guess.equals("BRITNEY"))
    {
     return(1);
    }
    else
   ...
   public void playSound()
   {
    ...
    try
    {
     BGM = new AudioStream(new FileInputStream("BRITNEYSPEARS.wav"));// Imports audio file
     MD =  BGM.getData();
	 ...
	 {
	  System.out.println("Error 401 File Not Found BRITNEY");// Exception if audio file is not found
	 }
   }   
